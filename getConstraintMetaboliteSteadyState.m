function [Aeq,beq] = getConstraintMetaboliteSteadyState(model)
% getConstraintMetaboliteSteadyState returns, given a deFBA model structure, 
% the equality constraint matrix and its corresponding right hand side 
% for imposing that at each time point internal metabolites are at steady
% state
%
%INPUTS
...
% Alexandra Reimers 14/07/2017
    ...
    Aeq = sparse(model.N*model.sizeXmet,size(toVector(cons,model),2));
    t = 1;
    for i = 1:model.N
        for l = 1:sizeXmet
            % compute constraint coefficient for vx and vy
            idx = getIndexVariable(model,'v',i,1:(sizeXrxn+model.sizeYrxn));
            Aeq(t,idx) = model.S(l,1:(sizeXrxn+model.sizeYrxn));
            % compute constraint coefficient for vp for quota rxns
            idx = getIndexVariable(model,'v',i,(sizeXrxn+model.sizeYrxn+1):(sizeXrxn+model.sizeYrxn+model.sizeQuotaRxn));
            Aeq(t,idx) = model.S(l,(sizeXrxn+model.sizeYrxn+1):(sizeXrxn+model.sizeYrxn+model.sizeQuotaRxn));
            % for enzyme production rxns
            idx = getIndexVariable(model,'v',i,(sizeXrxn+model.sizeYrxn+model.sizeQuotaRxn+1):(sizeXrxn+model.sizeYrxn+model.sizePrxn));
            Aeq(t,idx) = model.epsilon*model.S(l,(sizeXrxn+model.sizeYrxn+model.sizeQuotaRxn+1):end);

            t = t+1;
        end
    end
    beq = zeros(1,model.N*model.sizeXmet);
end
