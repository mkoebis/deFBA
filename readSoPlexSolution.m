function x = readSoPlexSolution(model,filename)
% readSoPlexSolution reads a deFBA solution returned by SoPlex from file.
%
...
% filename          String containing the name of the solution file
%
%OUTPUT
% x                 solution vector
%
% Alexandra Reimers 14/07/2017
    ...
    x = zeros(length(toVector(cons,model)),1);
    fid = fopen(filename);
    fgets(fid);
    tline = fgets(fid);
    
    while ischar(tline)
        mysplit = strsplit(tline);
        if strfind(mysplit{1},'x')==1
            s = mysplit{1};
            x(str2num(s(2:end))) = str2num(mysplit{2});
        end
        tline = fgets(fid); 
    end
    fclose(fid);
