function [Aineq,bineq] = getConstraintBiomassComposition(model)
...
% Alexandra Reimers 14/07/2017
    ...
    for j=1:length(model.quotaInitial)
        for i=1:model.N        
            ...
            Aineq(t,idx) = model.quotaWeights(j)*(model.quotaInitial(j)-1);
            
            qidx = [1:1:j-1, j+1:1:length(model.quotaInitial)];
            idx = getIndexVariable(model,'p',i,qidx);
            Aineq(t,idx) = model.quotaWeights(j)*model.quotaInitial(j);
            
            idx = getIndexVariable(model,'p',i,model.sizeQuotaMet+1:1:model.sizePmet);
            Aineq(t,idx) = model.proteinWeights*model.epsilon*model.quotaInitial(j);
            
            idx = getIndexVariable(model,'y',i,1:1:model.noStorage);
            Aineq(t,idx) = model.quotaInitial(j);
