function [result, strsol] = transformEpsilonScaling(result,model)
% transformDilutionResult scales back a solution that was obtained using a
% the scaling by model.epsilon of the macromolecule amounts and production 
% fluxes variables 
%
%INPUT
% result            deFBA solution obtained with the dilution term model.beta
...
%
%OUTPUT
% result            deFBA solution scaled back, i.e. without the scaling factor model.epsilon for macromolecules
% strsol            deFBA solution in struct form corresponding to the scaled back solution
%
% Alexandra Reimers 14/07/2017

    strsol = toStruct(result,model);
    
    strsol.v(:,model.sizeXrxn+model.sizeYrxn+model.sizeQuotaRxn+1:end) = strsol.v(:,model.sizeXrxn+model.sizeYrxn+model.sizeQuotaRxn+1:end)*model.epsilon;
    strsol.pdot(:, model.sizeQuotaMet+1:end) = strsol.pdot(:, model.sizeQuotaMet+1:end)*model.epsilon;
    strsol.p(:, model.sizeQuotaMet+1:end) = strsol.p(:, model.sizeQuotaMet+1:end)*model.epsilon;    
    strsol.p0(model.sizeQuotaMet+1:end) = strsol.p0(model.sizeQuotaMet+1:end)*model.epsilon;        
    
    result = toVector(strsol,model);
end