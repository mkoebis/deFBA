function [result, strsol] = transformDilutionResult(result,model)
% transformDilutionResult scales back a solution that was obtained using a
% dilution term for better numerics
%INPUT
...
%
%OUTPUT
% result            deFBA solution scaled back, i.e. without dilution term model.beta
...
% Alexandra Reimers 14/07/2017
    ...
    t = h:h:model.tf;
    for i=1:model.N
        strsol.v(i,1,:) = strsol.v(i,1,:)*exp(model.beta*t(i));
        strsol.vRev(i,1,:) = strsol.vRev(i,1,:)*exp(model.beta*t(i));
        strsol.ydot(i,1,:) = strsol.ydot(i,1,:)*exp(model.beta*t(i));
        strsol.pdot(i,1,:) = strsol.pdot(i,1,:)*exp(model.beta*t(i));
        
        strsol.y(i,:) = strsol.y(i,:)*exp(model.beta*t(i));
        strsol.p(i,:) = strsol.p(i,:)*exp(model.beta*t(i));
    end
